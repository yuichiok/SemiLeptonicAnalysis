...
class NewLeptonFinder : public Processor {
 ...
 protected:
  ...
  int _object_QuarkOrigin[nObjects]; // -1 if not comming from a quark, 0 for uds, 1 for c and 2 for b
  bool _objectFromtau[nObjects];
  bool _objectFromBmeson[nObjects];
  // PFOID
  ...
  //Not Isolated lepton information for semi-leptonic B decay

  int _nbNotIsoLep;
  int _NotIsoLepcharge[nObjects];
  int _NotIsoLepPDG[nObjects];
