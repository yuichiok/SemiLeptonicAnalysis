...
LeptonFinder::LeptonFinder() : Processor("LeptonFinder")
{
  ...
  registerOutputCollection( LCIO::RECONSTRUCTEDPARTICLE,
			    ...
			    _OutPFOscolName,
			    std::string("PhD_PFOsNoSelectedLepton"));
...
{ 
  ...
  _MCleptonReal = new TLorentzVector();
  _tree->Branch("MCleptonReal","TLorentzVector",&_MCleptonReal,16000,0);
...
{ 
  ...
  try {
    ...
    float _bField = Global::GEAR->getBField().at( gear::Vector3D( 0., 0., 0.) ).z();
    ...
    _MCleptonReal->SetPxPyPzE(0.,0.,0.,0.);
	...
	for(int i = 2 ; i < 8 ; i++)
	  {
		...
		if(abs( mcpart->getPDG() ) == 11 || abs( mcpart->getPDG() ) == 13 || abs( mcpart->getPDG() ) == 15)
		  {
		    ...
		    *_MClepton = mcVec;
		    ...
		    if(mcpart->getParents().size() != 0)
		      {
			for(unsigned int parent = 0; parent < mcpart->getParents().size(); parent++)
			  { 
			    if(abs((mcpart->getParents()[parent])->getPDG()) == 15) _isMCLeptonFromTau = true; 
			  }
		      }
		  }
	      }
	...
	if(leptonSeen)
	  {
	    for(int i = 8 ; i < mccol->getNumberOfElements() ; i++)
	      {
		if(!leptonTwoSeen)
		  {
		    MCParticle *mcpart = dynamic_cast<MCParticle*>( mccol->getElementAt(i) );
		    if(mcpart->getPDG() == _MCpdg && (mcpart->hasLeftDetector() || mcpart->isDecayedInCalorimeter() || mcpart->isDecayedInTracker()))
		      {
			leptonTwoSeen = true;
			bslepton = dynamic_cast<MCParticle*>( mccol->getElementAt(i) );
			TLorentzVector mcVec(TVector3(mcpart->getMomentum()),mcpart->getEnergy());
			*_MCleptonReal = mcVec;
		      }
		  }
	      }
	    if(!leptonTwoSeen) *_MCleptonReal = *_MClepton;
	  }
      }
	...
	if(leptonTwoSeen) 
	  {
	    if(findPFOs.getRelatedFromObjects(bslepton).size() == 1)
	      {
		std::cout << "findPFOs.getRelatedFromObjects(bslepton).size() == 1 OK" << std::endl;
		int pos = 0;
		ReconstructedParticle *recolepton = dynamic_cast<ReconstructedParticle*>( findPFOs.getRelatedFromObjects(bslepton).at(0) );
...
*/    
	...
	for(int k = 0; k < (int)jetParticles.size(); k++)
	  {
	    ...
	    if(findPFOs.getRelatedToObjects(jetParticles.at(k)).size() == 1)
	      {
		MCParticle *mcObj = dynamic_cast<MCParticle*>( findPFOs.getRelatedToObjects(jetParticles.at(k)).at(0) );
	    ...
	    // _TrackMom[pfopos] = jetParticles.at(k)->getEnergy()
		...
		helix->Initialize_Canonical( tkphi , tkd0 , tkz0 , tkomega , tktanlambda , _bField );
    ...
    // Will need to set photon to 0 later and recalculate z/pT/xT for leptons
    pfopos = 0;
    int thispos = 0;
    for(int i = 0; i < fourjetscol->getNumberOfElements(); i++)
      {
	    ...
	    if((abs(_object_PDG[pfopos]) == 11 || abs(_object_PDG[pfopos]) == 13) && _object_energy[pfopos] > _leptonEmin)
	      {
		_lepton_pos[thispos] = pfopos;
		TLorentzVector thisLepton(TVector3(jetParticles.at(k)->getMomentum()),jetParticles.at(k)->getEnergy());
		...
		for(int j = 0; j < fourjetscol->getNumberOfElements(); j++)
		  {
		    ...
		    for(int l = 0; l < (int)jetParticles2.size(); l++)
		      {
			int photonNb = 0;
			if(_object_PDG[pfopos2] == 22 && _object_energy[pfopos2] > _leptonEmin && photonNb < 5)
			  {
			    TLorentzVector thisPhoton(TVector3(jetParticles2.at(l)->getMomentum()),jetParticles2.at(l)->getEnergy());
			    float dist = 999.;
			    ...
			    if((fabs(cos(thisPhoton.Angle(thisLepton.Vect()))) > 0.999 && j == i) // case for FSRs
				 || ( dist < 20. &&  j == i && abs(_object_PDG[pfopos]) == 11) // simple case for bremsstrahlung
				 ) // then keep together
			      {
				thisLepton += thisPhoton;
				_object_energy[pfopos2] = 0.;
				_z4[pfopos2] = 0.;
				_pT[pfopos2] = 0.;
				_xT[pfopos2] = 0.;
		...
		_object_energy[pfopos] = thisLepton.E();
		_object_px[pfopos] = thisLepton.Px();
		_object_py[pfopos] = thisLepton.Py();
		_object_pz[pfopos] = thisLepton.Pz();
		_object_momentum[pfopos] = thisLepton.P();
		_object_phi[pfopos] = thisLepton.Phi();
		_object_theta[pfopos] = thisLepton.Theta();
		_object_jetNb[pfopos] = i;
		// realculate z4, pT4 and for each Lepton
		_z4[pfopos] = thisLepton.E()/Jet4vec.E();
		float theta = TMath::Abs(thisLepton.Angle(Jet4vec.Vect()));
		_pT[pfopos] = thisLepton.P()*sin(theta);
		_xT[pfopos] = _pT[pfopos]/Jet4vec.M();
		thispos++;
	      }
	    pfopos++;
    ...
    if(_objectFromtau[_leptonNb] == true) _lepFromtau = true;
    int photonIndex = -1;
